...
.readAcquFile <- function(fidFile, verbose=FALSE) {
    ...
    metaData$hpcLimits <- c(
        minMass=.grepAcquDoubleValue("##\\$HPClBLo=", acquLines),
        maxMass=.grepAcquDoubleValue("##\\$HPClBHi=", acquLines));
    metaData$hpcOrder <-
        as.double(.grepAcquValue("##\\$HPClOrd=", acquLines));
    metaData$hpcUse <- 
    ...
    metaData$digitizerType <- .grepAcquDoubleValue("##\\$DIGTYP=", acquLines);
    metaData$deflectionPulserCal1 <- 
...
## function .grepAcquDoubleValue
##  helper function to extract double values from acqu file
##
## params:
##  patternStr: pattern to look for
##  srcStr: where to look for patternStr
##
## returns:
##  double value of the value given in patternStr
##
